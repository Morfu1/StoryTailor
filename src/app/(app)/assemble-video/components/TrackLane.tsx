...
export default function TrackLane({
  ...
  return (
    ...
    >
      ...
      <div className={`flex-grow flex space-x-1 ${track.height} items-center overflow-hidden p-1`}>
        {track.items && track.items.length > 0 ? (
          track.items.map((item) => {
            ...
            const itemWidth = isEffectivelySelected ? "w-[240px]" : "w-[120px]"; // Example width

            if (track.type === 'video' && item.type === 'image' && item.imageUrl) {
              return (
                <div
                  key={item.id}
                  className={`flex-shrink-0 ${itemWidth} ${track.height} rounded-md overflow-hidden border-2 ${isEffectivelySelected ? "border-primary shadow-lg" : "border-transparent hover:border-primary/50"} cursor-pointer relative transition-all duration-200 ease-in-out group`}
                ...
                >
                  <div className="flex w-full h-full">
                    ...
                    />
                    {isEffectivelySelected && item.imageUrl && (
                      <Image
                        src={item.imageUrl}
              ...
              return (
                <div
                  key={item.id}
                  className={`flex-shrink-0 ${itemWidth} ${track.height} rounded-md p-1.5 border ${isEffectivelySelected ? "border-primary/30 bg-primary/10" : "border-border bg-muted/30"} overflow-hidden transition-all duration-200 ease-in-out cursor-pointer`}
                  onClick={(e) => { e.stopPropagation(); handleItemClick(item); }}
                >
              ...
              return (
                <div
                  key={item.id}
                  className={`flex-grow ${itemWidth} ${track.height} rounded-md flex items-center justify-center p-1 border ${isEffectivelySelected ? "border-primary bg-primary/10" : "border-border bg-muted/30"} cursor-pointer`}
                  onClick={(e) => { e.stopPropagation(); handleItemClick(item); }}
                >
